...
class FiftyOneAnnotationApp:
    ...
    def __init__(self, images_dir: str, dataset_name: str = "annotation_dataset",
                 complete_dataset_json: Optional[str] = None,
                 tag_categories: Optional[Dict[str, List[str]]] = None):
        """
        Initialize FiftyOne annotation application with tagging.

        Args:
            images_dir: Directory containing images (used when creating a fresh dataset)
            dataset_name: Name for the FiftyOne dataset
            complete_dataset_json: Path to complete_dataset.json to import from when the
                dataset is not already in MongoDB (e.g. on a new machine).
                Defaults to annotation_dataset/complete_dataset.json next to this script.
        ...
        self.complete_dataset_json = Path(complete_dataset_json) if complete_dataset_json \
            else script_dir / "annotation_dataset" / "complete_dataset.json"
    ...
    def _load_or_create_dataset(self) -> fo.Dataset:
        """Load existing dataset from MongoDB, or import from complete_dataset.json if available,
        or create a fresh empty dataset from the images directory as a last resort."""
        ...
        if self.complete_dataset_json.exists():
            print(f"Dataset not in MongoDB. Importing from {self.complete_dataset_json}...")
            return self._import_from_json(self.complete_dataset_json)

        print(f"No JSON found. Creating new empty dataset: {self.dataset_name}")
        dataset = fo.Dataset(self.dataset_name)
        dataset.persistent = True
        samples = [
            fo.Sample(filepath=str(p))
            for ext in ['.jpg', '.jpeg', '.png', '.bmp', '.tiff']
            for p in list(self.images_dir.glob(f"*{ext}")) + list(self.images_dir.glob(f"*{ext.upper()}"))
        ]
        if samples:
    ...
    def _import_from_json(self, json_path: Path) -> fo.Dataset:
        """Import dataset from complete_dataset.json, resolving images via images_dir."""
        ...
        for s in data["samples"]:
            # Prefer images_dir override, fall back to original_filepath from JSON
            img_path = self.images_dir / s["filename"]
        ...
        # Bulk-set classification fields (one write per field, not per sample)
        ...
        print(f"Imported {len(samples)} samples from {json_path.name} (skipped {len(skipped)}).")
        return dataset
    ...
    def export_annotations(self, output_dir: str, format: str = "yolo",
        ...
        if format.lower() == "yolo":
            ...
            self._export_fiftyone_with_tags(annotated_view, output_dir, include_tags)
...
"""
    ...
    def _export_fiftyone_with_tags(self, view: fo.DatasetView, output_dir: Path, 
                            include_tags: bool):
        # Use FiftyOne's fiftyone export
        view.export(
            export_dir=str(output_dir),
            dataset_type=fo.types.fo.types.FiftyOneImageDetectionDataset,
            label_field="ground_truth"
        )
        
        # Add tags to COCO JSON
        if include_tags:
            fiftyone_json_path = output_dir / "labels.json"
            
            if fiftyone_json_path.exists():
                with open(fiftyone_json_path, 'r') as f:
                    fiftyone_data = json.load(f)
                ...
                for sample in view:
                    ...
                    # Find image in COCO data
                    for img_entry in fiftyone_data['images']:
                        if img_entry['file_name'] == img_filename:
                            img_entry['tags'] = tags
                            break
                
                # Save updated COCO JSON
                with open(fiftyone_json_path, 'w') as f:
                    json.dump(fiftyone_data, f, indent=2)
...
Examples:
  ...
  python fiftyone_annotation_app_fiftyoneschema.py /path/to/images --model yolov5
  
  # Import auto-generated tags from Places365
  python fiftyone_annotation_app_fiftyoneschema.py /path/to/images --import-tags tags.json --model faster-rcnn
  ...
  python fiftyone_annotation_app_fiftyoneschema.py /path/to/images --mark-high-conf-fixed --confidence 0.8
  
  # Export only reviewed/fixed samples
  python fiftyone_annotation_app_fiftyoneschema.py /path/to/images --export output --only-fixed
  
  # Reset all review statuses back to todo
  python fiftyone_annotation_app_fiftyoneschema.py /path/to/images --reset-review-status
...
Tag Categories:
  ...
  review_status: todo, fixed, needs_work, reviewed, skip
    ...
    parser.add_argument('images_dir', help='Directory containing images e.g. ../datasets/VisDrone2019-DET-train/images/')
    parser.add_argument('--dataset-name', default='annotation_dataset',
                       help='Name for FiftyOne dataset (default: annotation_dataset)')
    parser.add_argument('--json', default="../datasets/VisDrone2019-DET-train/complete_dataset.json", dest='complete_dataset_json',
                       help='Path to complete_dataset.json to import from on a new machine '
                            '(default: ../datasets/VisDrone2019-DET-train/complete_dataset.json)')
    ...
    try:
        app = FiftyOneAnnotationApp(
            ...
            complete_dataset_json=args.complete_dataset_json,
        )
    except Exception as e:
